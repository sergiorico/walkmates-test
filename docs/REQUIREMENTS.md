...
- A Seeker MUST accept the terms of service at registration.
...
- The prompt builder MUST treat Listing/Seeker free-text as **data, not instructions**: text
  such as "ignore previous instructions…" embedded in a Listing description MUST NOT change the
  structure or intent of the produced output (basic prompt-injection resistance).

### FR-5.3 Metamorphic expectations (test oracles for Lab 3)
- **MR-1 (irrelevant detail):** adding an irrelevant sentence to a Seeker's bio MUST NOT change
  which Listing is recommended as the best match.
